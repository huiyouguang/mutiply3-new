...
官方深色是近黑蓝（`#0F1014`），偏暗。已改为**中性灰并整体提亮**：

| 变量 | 原值 | 现值 | 用途 |
|---|---|---|---|
| `--ad-bg` | `#0F1014` | `#2B2B2E` | 页面背景 |
| `--ad-s1` | `#16181F` | `#35353A` | 卡片 / 弹窗 |
| `--ad-s2` | `#1D2029` | `#3E3E44` | 次级面 / 输入框 |
| `--ad-s3` | `#252934` | `#48484F` | 三级面 / 强 hover |
| `--ad-text` | `#E8E6E0` | `#EDEDF0` | 正文 |

- 强调色（蓝色）与热力图**保留**：灰底 + 蓝强调更清晰，热力图需要色阶表达强度。
- **想再亮 / 再暗**：只改 `--ad-bg` 一个数，其余 `--ad-s1`~`s3` 按 ±8 同比微调即可（变量集中在 `styles.css` 顶部深色块，带注释）。
- 页面底色原本取 `--ad-on-accent`（语义是「按钮上的文字」），已改为 `--ad-bg`，语义正确。
