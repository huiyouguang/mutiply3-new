...
开发：蚕子 QQ：312815311 更新时间：2021-11-12
***************************************************************************** */
...
var MyPlugin = /** @class */ (function (_super) {
        ...
        function() {
            ...
            function(_a) {
                ...
                _this.addCommand({
		            id: 'internal-link2',
		            name: '[[链接|同名]]语法',
		            callback: function () {	_this.转换内链语法2();}
		        });
                _this.addCommand({
		            id: 'biaoti0-text',
		            name: '取消标题语法',
                ...
                _this.addCommand({
		            id: 'add-line1',
		            name: '当前行上新行',
		            callback: function() { _this.当前行上空行(); },
		            hotkeys: [{ modifiers: ["Shift","Alt"], key: "o" } ]    
		        });
                _this.addCommand({
		            id: 'add-line2',
		            name: '当前行下新行',
		            callback: function() { _this.当前行下空行(); },
    ...
    MyPlugin.prototype.转换内链语法2 = function() {
    	var _defaultChar = this.settings.defaultChar;
        var lines = this.获取所选文本 (this.获取编辑模式 ());
    ...
    MyPlugin.prototype.当前行上空行 = function(_str) {
        var mdView = this.app.workspace.activeLeaf.view;
        if (mdView.sourceMode == undefined)
            return false;
        var editor = mdView.sourceMode.cmEditor;
    ...
    MyPlugin.prototype.当前行下空行 = function(_str) {
        var mdView = this.app.workspace.activeLeaf.view;
        if (mdView.sourceMode == undefined)
            return false;
        var editor = mdView.sourceMode.cmEditor;
...
var SettingsTab = /** @class */ (function (_super) {
    ...
    SettingsTab.prototype.display = function () {
        ...
        containerEl.empty();
        containerEl.createEl('h2', { text: '增强编辑 0.3.2' });
        ...
        donateText.appendText('【转换内链语法2】　将选文转换为 [[|选文]] 样式后再选择文档')
        ...
        donateText.appendChild(document.createElement('br'));
        donateText.appendText('【转换待办状态】　转换选文行首的待办状态，顺序为[ x-!?><+]');
        donateText.appendChild(document.createElement('br'));
        ...
        donateText.appendChild(document.createElement('br'));
        donateText.appendText('【当前行上新行　　Shift+Alt+O】　在当前文本行的上行插入空白行');
        donateText.appendChild(document.createElement('br'));
        donateText.appendText('【当前行下新行　　Shift+Alt+L】　在当前文本行的下行插入空白行');
        donateText.appendChild(document.createElement('br'));
