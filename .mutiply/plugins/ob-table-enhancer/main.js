...
var getMoveColRightItem = (plugin, cellEl) => (item) => {
  ...
  item.onClick(async (e) => {
    ...
    if (j == colNum - 1) {
      new import_obsidian2.Notice("当前列已经是最右列。");
      return;
    }
...
var getMoveColLeftItem = (plugin, cellEl) => (item) => {
  ...
  item.onClick(async (e) => {
    ...
    if (j == 0) {
      new import_obsidian2.Notice("当前列已经是最左列。");
      return;
    }
    await plugin.tableEditor.swapCols(table, j, j - 1);
  });
...
var getMoveRowDownItem = (plugin, cellEl) => (item) => {
  ...
  item.onClick(async (e) => {
    ...
    const rowNum = table.cells.length;
    if (i == rowNum - 1) {
      new import_obsidian2.Notice("当前列已经是底行。");
      return;
    }
...
var getMoveRowUpItem = (plugin, cellEl) => (item) => {
  ...
  item.onClick(async (e) => {
    ...
    if (i == 1) {
      new import_obsidian2.Notice("当前列已经是顶行。");
      return;
    }
    await plugin.tableEditor.swapRows(table, i, i - 1);
  });
...
var ToolBar = class {
  constructor(plugin) {
    ...
    this.rowOpBarEl.createDiv({
      ...
      el.onclick = async () => {
        if (this.cell.i == 1) {
          new import_obsidian3.Notice("当前列已经是顶行。");
          return;
        }
    ...
    this.rowOpBarEl.createDiv({
      ...
      el.onclick = async () => {
        ...
        if (this.cell.i == rowNum - 1) {
          new import_obsidian3.Notice("当前列已经是底行。");
          return;
        }
        await this.plugin.tableEditor.swapRows(table, this.cell.i, this.cell.i + 1);
      };
    ...
    this.colOpBarEl.createDiv({
      ...
      el.onclick = async () => {
        if (this.cell.j == 0) {
          new import_obsidian3.Notice("当前列已经是最左列。");
          return;
        }
        const table = await plugin.tableEditor.getTable(this.cell.tableLine);
        if (!table) {
    ...
    this.rowOpBarEl.createDiv({
      ...
      el.onclick = async () => {
        if (this.cell.i == 0) {
          new import_obsidian3.Notice("无法删除表格的表头。");
          return;
        }
        const table = await plugin.tableEditor.getTable(this.cell.tableLine);
        if (!table) {
    ...
    this.colOpBarEl.createDiv({
      ...
      el.onclick = async () => {
        ...
        if (this.cell.j == colNum - 1) {
          new import_obsidian3.Notice("当前列已经是最右列。");
          return;
        }
